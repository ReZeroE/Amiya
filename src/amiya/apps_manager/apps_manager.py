...
class AppsManager:
    ...
    def delete_app(self, tag: str = None):
        ...
        if tag == None:                             # If no application tag is inputted by user
            self.print_apps()
            app_id_user_input = input(atext(f"Which app would you like to DELETE? (0-{len(self.apps)-1}) "))
    ...
    def add_tag(self):
        ...
        self.print_apps()
        app_id_user_input = input(atext(f"Which app would you like to ADD a tag to? (0-{len(self.apps)-1}) "))
        app = self.get_app_with_id(app_id_user_input)                        
